...
final public class BusinessAdministrator extends CachedObjectStringKey<BusinessAdministrator> implements PasswordProtected, Removable, Disablable {
	...
	password=in.readUTF();
	name=in.readUTF();
	title=readNullUTF(in);
	birthday=in.readLong();
	isPreferred=in.readBoolean();
	isPrivate=in.readBoolean();
	created=in.readLong();
	work_phone=in.readUTF();
	home_phone=readNullUTF(in);
	cell_phone=readNullUTF(in);
	fax=readNullUTF(in);
	email=in.readUTF();
	address1=readNullUTF(in);
	address2=readNullUTF(in);
	city=readNullUTF(in);
	state=readNullUTF(in);
	country=readNullUTF(in);
	zip=readNullUTF(in);
	...
	writeNullUTF(out, cell_phone);
	writeNullUTF(out, fax);
	out.writeUTF(email);
	writeNullUTF(out, address1);
	writeNullUTF(out, address2);
	writeNullUTF(out, city);
	writeNullUTF(out, state);
	writeNullUTF(out, country);
