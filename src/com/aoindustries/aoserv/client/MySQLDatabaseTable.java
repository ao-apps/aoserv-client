...
final public class MySQLDatabaseTable extends CachedTableIntegerKey<MySQLDatabase> {
    ...
    public static String isValidDatabaseName(Locale userLocale, String name, List<?> reservedWords) {
        ...
        int len = name.length();
        if(len==0) return ApplicationResourcesAccessor.getMessage(userLocale, "MySQLDatabaseTable.isValidDatabaseName.empty");
        if(len>MySQLDatabase.MAX_DATABASE_NAME_LENGTH) return ApplicationResourcesAccessor.getMessage(userLocale, "MySQLDatabaseTable.isValidDatabaseName.tooLong", len, MySQLDatabase.MAX_DATABASE_NAME_LENGTH);
        // The first character must be [a-z]
        char ch = name.charAt(0);
        if (ch < 'a' || ch > 'z') return ApplicationResourcesAccessor.getMessage(userLocale, "MySQLDatabaseTable.isValidDatabaseName.firstChar");
        // The rest may have additional characters
        for (int c = 1; c < len; c++) {
            ch = name.charAt(c);
            if ((ch < 'a' || ch > 'z') && (ch < '0' || ch > '9') && ch != '_') return ApplicationResourcesAccessor.getMessage(userLocale, "MySQLDatabaseTable.isValidDatabaseName.invalidChar", ch);
        }
        ...
        for(int c=0;c<size;c++) {
            ...
            if(name.equalsIgnoreCase(reservedWord)) return ApplicationResourcesAccessor.getMessage(userLocale, "MySQLDatabaseTable.isValidDatabaseName.reservedWord", reservedWord);
        }
    	return null;
    }
