...
public final class EmailSmtpRelayTable extends CachedTableIntegerKey<EmailSmtpRelay> {
    ...
    private static final OrderBy[] defaultOrderBy = {
        ...
        new OrderBy(EmailSmtpRelay.COLUMN_ACCOUNTING_name, ASCENDING)
    };
    @Override
    ...
    int addEmailSmtpRelay(
        final Business bu,
        final AOServer aoServer,
        final String host,
        final EmailSmtpRelayType type,
        final long duration
    ...
    List<EmailSmtpRelay> getEmailSmtpRelays(Business bu) throws IOException, SQLException {
        return getIndexedRows(EmailSmtpRelay.COLUMN_ACCOUNTING, bu.pkey);
    }

    EmailSmtpRelay getEmailSmtpRelay(Business bu, AOServer ao, String host) throws IOException, SQLException {
        int aoPKey=ao.pkey;

        // Use index first
        List<EmailSmtpRelay> cached = getEmailSmtpRelays(bu);
        int len = cached.size();
        for (int c = 0; c < len; c++) {
