...
final public class TimeoutConnectorFactory extends WrappedConnectorFactory<TimeoutConnector,TimeoutConnectorFactory> {
    ...
    @Override
    protected TimeoutConnector newWrappedConnector(final Locale locale, final UserId username, final String password, final UserId switchUser, final DomainName daemonServer, final boolean readOnly) throws LoginException, RemoteException {
        Future<TimeoutConnector> future = executorService.submit(
            new Callable<TimeoutConnector>() {
                ...
                public TimeoutConnector call() throws RemoteException, LoginException {
                    return new TimeoutConnector(TimeoutConnectorFactory.this, locale, username, password, switchUser, daemonServer, readOnly);
                }
            }
        );
        try {
