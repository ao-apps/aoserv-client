...
final public class TicketActionType extends GlobalObjectStringKey<TicketActionType> {
    ...
    String generateSummary(AOServConnector connector, Locale userLocale, String oldValue, String newValue) throws IOException, SQLException {
        if(oldValue==null) {
            if(newValue==null) return ApplicationResources.accessor.getMessage(userLocale, "TicketActionType."+pkey+".generatedSummary.null.null");
            return ApplicationResources.accessor.getMessage(userLocale, "TicketActionType."+pkey+".generatedSummary.null.notNull", newValue);
        } else {
            if(newValue==null) return ApplicationResources.accessor.getMessage(userLocale, "TicketActionType."+pkey+".generatedSummary.notNull.null", oldValue);
            return ApplicationResources.accessor.getMessage(userLocale, "TicketActionType."+pkey+".generatedSummary.notNull.notNull", oldValue, newValue);
        }
    }
}
