 ...
 */
final public class PostgresServerName implements
	Comparable<PostgresServerName>,
	Serializable,
	ObjectInputValidation,
	DtoFactory<com.aoindustries.aoserv.client.dto.PostgresServerName>,
	Internable<PostgresServerName>
{
	...
	public static ValidationResult validate(String name) {
		if(name==null) return new InvalidResult(ApplicationResources.accessor, "PostgresServerName.validate.isNull");
		int len = name.length();
		if(len==0) return new InvalidResult(ApplicationResources.accessor, "PostgresServerName.validate.isEmpty");
		if(len > MAX_LENGTH) return new InvalidResult(ApplicationResources.accessor, "PostgresServerName.validate.tooLong", MAX_LENGTH, len);
	...
	public static PostgresServerName valueOf(String name) throws ValidationException {
		...
		return new PostgresServerName(name, true);
	}

	final private String name;

	private PostgresServerName(String name, boolean validate) throws ValidationException {
		this.name = name;
	 ...
	 */
	private PostgresServerName(String name) {
