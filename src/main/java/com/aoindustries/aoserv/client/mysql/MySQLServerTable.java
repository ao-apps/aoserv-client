...
final public class MySQLServerTable extends CachedTableIntegerKey<MySQLServer> {
	...
	public MySQLServerTable(AOServConnector connector) {
	...
	int addMySQLServer(
		...
		if(!version.getTechnologyName_name().equals(TechnologyName.MYSQL)) throw new SQLException("TechnologyVersion must have name of "+TechnologyName.MYSQL+": "+version.getTechnologyName_name());
	...
	public MySQLServer getMySQLServer(NetBind nb) throws IOException, SQLException {
		return getUniqueRow(MySQLServer.COLUMN_BIND, nb.getId());
	}

	public List<MySQLServer> getMySQLServers(AOServer ao) throws IOException, SQLException {
		return getIndexedRows(MySQLServer.COLUMN_AO_SERVER, ao.getPkey());
	}

	public MySQLServer getMySQLServer(MySQLServerName name, AOServer ao) throws IOException, SQLException {
		// Use the index first
	...
	public boolean isMySQLServerNameAvailable(MySQLServerName name, AOServer ao) throws IOException, SQLException {
