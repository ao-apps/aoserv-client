...
final public class HttpdSite extends CachedObjectIntegerKey<HttpdSite> implements Disablable, Removable {
	...
	public LinuxServerAccount getLinuxServerAccount() throws SQLException, IOException {
		// May be filtered
		LinuxAccount obj = table.getConnector().getLinuxAccounts().get(linuxAccount);
	...
	public LinuxServerGroup getLinuxServerGroup() throws SQLException, IOException {
		LinuxGroup obj = table.getConnector().getLinuxGroups().get(linuxGroup);
	...
	public TechnologyVersion getPhpVersion() throws IOException, SQLException {
		...
		TechnologyVersion tv = table.getConnector().getTechnologyVersions().get(phpVersion);
		if(tv == null) throw new SQLException("TechnologyVersion not found: " + phpVersion);
		if(!tv.getTechnologyName_name().equals(TechnologyName.PHP)) throw new SQLException("Not a PHP version: " + tv.getTechnologyName_name() + " #" + tv.getPkey());
		if(
	...
	public int addHttpdSiteAuthenticatedLocation(
		...
		return table.getConnector().getHttpdSiteAuthenticatedLocationTable().addHttpdSiteAuthenticatedLocation(
			this,
			path,
			isRegularExpression,
			authName,
	...
	public boolean canDisable() throws IOException, SQLException {
		...
		for(HttpdSiteBind hsb : getHttpdSiteBinds()) if(!hsb.isDisabled()) return false;
		return true;
	}
	...
	public void disable(DisableLog dl) throws IOException, SQLException {
		table.getConnector().requestUpdateIL(true, AOServProtocol.CommandID.DISABLE, SchemaTable.TableID.HTTPD_SITES, dl.getPkey(), pkey);
	}

	@Override
	public void enable() throws IOException, SQLException {
		table.getConnector().requestUpdateIL(true, AOServProtocol.CommandID.ENABLE, SchemaTable.TableID.HTTPD_SITES, pkey);
	}
	...
	public List<HttpdSiteAuthenticatedLocation> getHttpdSiteAuthenticatedLocations() throws IOException, SQLException {
		return table.getConnector().getHttpdSiteAuthenticatedLocationTable().getHttpdSiteAuthenticatedLocations(this);
	}
	...
	public List<HttpdSiteBind> getHttpdSiteBinds(HttpdServer server) throws SQLException, IOException {
		return table.getConnector().getHttpdSiteBinds().getHttpdSiteBinds(this, server);
	}

	public HttpdStaticSite getHttpdStaticSite() throws IOException, SQLException {
		return table.getConnector().getHttpdStaticSites().get(pkey);
	}

	public HttpdTomcatSite getHttpdTomcatSite() throws IOException, SQLException {
		return table.getConnector().getHttpdTomcatSites().get(pkey);
	}
	...
	public HttpdSiteURL getPrimaryHttpdSiteURL() throws SQLException, IOException {
		...
		Port httpPort = table.getConnector().getProtocols().get(Protocol.HTTP).getPort();
		Port httpsPort = table.getConnector().getProtocols().get(Protocol.HTTPS).getPort();
	...
	public void remove() throws IOException, SQLException {
		table.getConnector().requestUpdateIL(true, AOServProtocol.CommandID.REMOVE, SchemaTable.TableID.HTTPD_SITES, pkey);
	}
