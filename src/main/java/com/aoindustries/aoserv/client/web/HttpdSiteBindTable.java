...
final public class HttpdSiteBindTable extends CachedTableIntegerKey<HttpdSiteBind> {
	...
	public HttpdSiteBindTable(AOServConnector connector) {
		super(connector, HttpdSiteBind.class);
	}

	private static final OrderBy[] defaultOrderBy = {
		new OrderBy(HttpdSiteBind.COLUMN_HTTPD_SITE_name+'.'+HttpdSite.COLUMN_NAME_name, ASCENDING),
		...
		new OrderBy(HttpdSiteBind.COLUMN_HTTPD_BIND_name+'.'+HttpdBind.COLUMN_NET_BIND_name+'.'+NetBind.COLUMN_IP_ADDRESS_name+'.'+IPAddress.COLUMN_DEVICE_name+'.'+NetDevice.COLUMN_DEVICE_ID_name, ASCENDING),
		new OrderBy(HttpdSiteBind.COLUMN_HTTPD_BIND_name+'.'+HttpdBind.COLUMN_NET_BIND_name+'.'+NetBind.COLUMN_PORT_name, ASCENDING),
		new OrderBy(HttpdSiteBind.COLUMN_NAME_name, ASCENDING)
	};
	@Override
	...
	List<HttpdSiteBind> getHttpdSiteBinds(HttpdSite site) throws IOException, SQLException {
		return getIndexedRows(HttpdSiteBind.COLUMN_HTTPD_SITE, site.getPkey());
	}
	...
	public List<HttpdSiteBind> getHttpdSiteBinds(SslCertificate sslCert) throws IOException, SQLException {
		return getIndexedRows(HttpdSiteBind.COLUMN_SSL_CERTIFICATE, sslCert.getPkey());
	}
