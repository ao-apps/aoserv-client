...
public class TicketLoggingHandler extends QueuedHandler {
  ...
  protected void backgroundPublish(Formatter formatter, LogRecord rec, String fullReport) throws IOException, SQLException {
    ...
    if (brand == null) {
      throw new SQLException("Unable to find Brand for connector: " + connector);
    }
    ...
    if (ticketType == null) {
      throw new SQLException("Unable to find TicketType: " + TicketType.LOGS);
    }
    Category category;
    if (categoryDotPath != null) {
      category = connector.getReseller().getCategory().getTicketCategoryByDotPath(categoryDotPath);
      if (category == null) {
        throw new SQLException("Unable to find Category: " + categoryDotPath);
      }
    } else {
      category = null;
    }
