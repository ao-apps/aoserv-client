...
final public class LinuxServerAccountTable extends CachedTableIntegerKey<LinuxServerAccount> {
	...
	public LinuxServerAccountTable(AOServConnector connector) {
	...
	private final Map<Integer,Map<UserId,LinuxServerAccount>> nameHash=new HashMap<>();

	LinuxServerAccount getLinuxServerAccount(AOServer aoServer, UserId username) throws IOException, SQLException {
		synchronized(nameHash) {
			if(!nameHashBuilt) {
				nameHash.clear();
				...
				for(int c=0; c<len; c++) {
					...
					Map<UserId,LinuxServerAccount> serverHash=nameHash.get(I);
			...
			Map<UserId,LinuxServerAccount> serverHash=nameHash.get(aoServer.getPkey());
	 ...
	 */
	public LinuxServerAccount getLinuxServerAccountFromUsernamePassword(UserId username, String password, boolean emailOnly) throws LoginException, IOException, SQLException {
		List<LinuxServerAccount> list = getRows();
	...
	LinuxServerAccount getLinuxServerAccount(AOServer aoServer, LinuxId uid) throws IOException, SQLException {
		synchronized(uidHash) {
			...
			Map<LinuxId,LinuxServerAccount> serverHash=uidHash.get(aoServer.getPkey());
			if(serverHash==null) return null;
	...
	List<LinuxServerAccount> getLinuxServerAccounts(LinuxAccount linuxAccount) throws IOException, SQLException {
		return getIndexedRows(LinuxServerAccount.COLUMN_USERNAME, linuxAccount.getUsername_id());
	}

	List<LinuxServerAccount> getLinuxServerAccounts(AOServer aoServer) throws IOException, SQLException {
		return getIndexedRows(LinuxServerAccount.COLUMN_AO_SERVER, aoServer.getPkey());
	}
