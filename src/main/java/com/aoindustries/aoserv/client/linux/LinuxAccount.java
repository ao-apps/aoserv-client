...
final public class LinuxAccount extends CachedObjectUserIdKey<LinuxAccount> implements PasswordProtected, Removable, Disablable {
	...
	public boolean canDisable() throws IOException, SQLException {
		...
		for(LinuxServerAccount lsa : getLinuxServerAccounts()) if(!lsa.isDisabled()) return false;
	...
	public void disable(DisableLog dl) throws IOException, SQLException {
		table.getConnector().requestUpdateIL(true, AOServProtocol.CommandID.DISABLE, SchemaTable.TableID.LINUX_ACCOUNTS, dl.getPkey(), pkey);
	}

	@Override
	public void enable() throws IOException, SQLException {
		table.getConnector().requestUpdateIL(true, AOServProtocol.CommandID.ENABLE, SchemaTable.TableID.LINUX_ACCOUNTS, pkey);
	}
	...
	public LinuxAccountType getType() throws IOException, SQLException {
		LinuxAccountType typeObject = table.getConnector().getLinuxAccountTypes().get(type);
	...
	public void removeLinuxGroup(LinuxGroup group) throws IOException, SQLException {
		for(LinuxGroupAccount lga : table.getConnector().getLinuxGroupAccounts().getLinuxGroupAccounts(group.getName(), pkey)) {
			lga.remove();
		}
	}
	...
	public void setPrimaryLinuxGroup(LinuxGroup group) throws SQLException, IOException {
		List<LinuxGroupAccount> lgas = table.getConnector().getLinuxGroupAccounts().getLinuxGroupAccounts(group.getName(), pkey);
