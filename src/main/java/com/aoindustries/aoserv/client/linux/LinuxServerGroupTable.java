...
final public class LinuxServerGroupTable extends CachedTableIntegerKey<LinuxServerGroup> {
	...
	public LinuxServerGroupTable(AOServConnector connector) {
		...
		for (int c = 0; c < len; c++) {
			...
			if (aoPKey==group.ao_server) {
				...
				if (pk!=null && pk.getBusiness_accounting().equals(accounting)) {
					...
					if (linuxGroup.getLinuxGroupType().getName().equals(LinuxGroupType.USER)) return group;
				}
			}
		}
		return null;
	...
	public LinuxServerGroup getLinuxServerGroup(AOServer aoServer, GroupId group_name) throws IOException, SQLException {
		synchronized(nameHash) {
			if(!nameHashBuilt) {
				nameHash.clear();
			...
			Map<GroupId,LinuxServerGroup> serverHash=nameHash.get(aoServer.getPkey());
	...
	public LinuxServerGroup getLinuxServerGroup(AOServer aoServer, LinuxId gid) throws IOException, SQLException {
		synchronized(gidHash) {
			...
			Map<LinuxId,LinuxServerGroup> serverHash=gidHash.get(aoServer.getPkey());
			if(serverHash==null) return null;
	...
	List<LinuxServerGroup> getLinuxServerGroups(AOServer aoServer) throws IOException, SQLException {
		return getIndexedRows(LinuxServerGroup.COLUMN_AO_SERVER, aoServer.getPkey());
	}

	List<LinuxServerGroup> getLinuxServerGroups(LinuxGroup lg) throws IOException, SQLException {
		return getIndexedRows(LinuxServerGroup.COLUMN_NAME, lg.getName());
	}
	...
	LinuxServerGroup getPrimaryLinuxServerGroup(LinuxServerAccount account) throws SQLException, IOException {
		...
		LinuxServerGroup lsg=getLinuxServerGroup(account.getAOServer(), linuxGroup.getName());
