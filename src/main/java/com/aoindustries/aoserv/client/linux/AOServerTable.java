...
final public class AOServerTable extends CachedTableIntegerKey<AOServer> {
	...
	public AOServerTable(AOServConnector connector) {
	...
	public AOServer getAOServerByDaemonNetBind(NetBind nb) throws IOException, SQLException {
	...
	public AOServer getAOServerByJilterNetBind(NetBind nb) throws IOException, SQLException {
		int bind_id=nb.getId();
		List<AOServer> servers=getRows();
		int size=servers.size();
		for(int c=0;c<size;c++) {
