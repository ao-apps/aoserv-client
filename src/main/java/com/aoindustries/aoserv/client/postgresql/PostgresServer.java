...
final public class PostgresServer extends CachedObjectIntegerKey<PostgresServer> {
	...
	public NetBind getBind() throws SQLException, IOException {
		NetBind nb = table.getConnector().getNetBinds().get(pkey);
	...
	public PostgresVersion getVersion() throws SQLException, IOException {
		PostgresVersion obj=table.getConnector().getPostgresVersions().get(version);
	...
	public int addPostgresDatabase(
		...
		return table.getConnector().getPostgresDatabases().addPostgresDatabase(
			name,
			this,
			datdba,
			encoding,
	...
	public PostgresDatabase getPostgresDatabase(PostgresDatabaseName name) throws IOException, SQLException {
		return table.getConnector().getPostgresDatabases().getPostgresDatabase(name, this);
	}
	...
	public PostgresServerUser getPostgresServerUser(PostgresUserId username) throws IOException, SQLException {
		return table.getConnector().getPostgresServerUsers().getPostgresServerUser(username, this);
	}
	...
	public boolean isPostgresDatabaseNameAvailable(PostgresDatabaseName name) throws IOException, SQLException {
		return table.getConnector().getPostgresDatabases().isPostgresDatabaseNameAvailable(name, this);
	}
